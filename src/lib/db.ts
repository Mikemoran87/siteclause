...
export async function clearVariationsBySource(projectId: string, source: 'contract' | 'programme'): Promise<void> {
  await supabase.from('variations').delete().eq('project_id', projectId).eq('source', source)
}
