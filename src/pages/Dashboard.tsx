    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <nav className="bg-[#1B4332] px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-4">
          <a href="/demo" className="text-green-300 text-sm hover:text-white transition-colors">
            Demo
          </a>
          <button
            onClick={handleSignOut}
            className="text-green-300 text-sm hover:text-white transition-colors border border-green-700 rounded-lg px-3 py-1.5"
          >
            Sign Out
          </button>
        </div>
      ...
      <div className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">Track variations, deadlines, and contract claims</p>
          </div>
          <button
            onClick={() => { setShowNewForm(true); setFormError('') }}
            className="bg-[#1B4332] hover:bg-[#2D6A4F] text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-colors shadow-sm"
          >
            + New Project
          </button>
        </div>
            ...
            <form onSubmit={handleCreate} className="p-6">
              ...
              <div className="mt-4 flex items-center justify-end gap-3">
                <button
                  type="button"
                  onClick={() => setShowNewForm(false)}
                  className="text-sm text-gray-500 hover:text-gray-700 border border-gray-200 rounded-xl px-4 py-2.5"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={submitting}
                  className="bg-[#1B4332] hover:bg-[#2D6A4F] disabled:opacity-50 text-white font-bold px-5 py-2.5 rounded-xl text-sm transition-colors"
                >
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <p className="text-gray-400 text-sm mb-6">Create your first project to start tracking contract claims.</p>
            <button
              onClick={() => setShowNewForm(true)}
              className="bg-[#1B4332] text-white font-bold px-6 py-3 rounded-xl text-sm"
            >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {projects.map((p) => (
              <div
                ...
                className="bg-white rounded-2xl border border-gray-200 p-5 shadow-sm hover:shadow-md hover:border-[#1B4332] cursor-pointer transition-all group"
              >
                ...
                {p.main_contractor && (
                  <p className="text-xs text-gray-500 mb-3">{p.main_contractor}</p>
                )}
